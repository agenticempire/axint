...
function formatOutput(swift: string, inputTokens: number): WorkerToolResult {
  const out = Math.ceil(swift.length / 4);
  const ratio = inputTokens > 0 ? (out / inputTokens).toFixed(2) : "0.00";
  const saved = inputTokens - out;
  const text = `// In: ~${inputTokens}  Out: ~${out}  Ratio: ${ratio}x  Saved: ${saved > 0 ? `+${saved}` : saved}\n\n${swift}`;
  return {
    content: [
      {
        type: "text",
    ...
    structuredContent: { text, isError: false },
  };
}

function textResult(text: string, isError = false): WorkerToolResult {
  return {
    content: [{ type: "text", text }],
    structuredContent: { text, isError },
    ...(isError ? { isError: true } : {}),
  };
}

function withStructuredText(result: WorkerToolResult): WorkerToolResult {
...
function runWorkerCloudCheck(args: Record<string, unknown>) {
    ...
    diagnostics.length === 0
      ...
      : [
          ...
          ...diagnostics
            ...
            .map(
              (d) =>
                `- ${d.code}: ${d.message}${d.suggestion ? ` Fix: ${d.suggestion}` : ""}`
            ),
        ].join("\n");
  ...
  if (format === "json")
    return textResult(JSON.stringify(payload, null, 2), status === "fail");
...
function runWorkerTokenIngest(args: Record<string, unknown>) {
  ...
  const colors = Object.entries(tokens).filter(([, v]) =>
    /^#(?:[0-9a-f]{3}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(String(v))
  );
  const layout = Object.entries(tokens).filter(([k]) =>
    /(sidebar|rail|column|width|height|layout)/i.test(k)
  );
  const lines = [
  ...
  if (colors.length) {
    ...
    for (const [k, v] of colors)
      lines.push(`        static let ${swiftIdentifier(k)} = Color(hex: "${String(v)}")`);
    lines.push("    }", "");
  }
  if (layout.length) {
    ...
    for (const [k, v] of layout)
      lines.push(
        `        static let ${swiftIdentifier(k)} = CGFloat(${String(v).replace(/px$/, "")})`
      );
    lines.push("    }", "");
  }
  lines.push("}");
...
function parseInlineWorkerTokens(
  source: string
...
function flattenWorkerTokens(
  ...
  if (typeof value === "object" && !Array.isArray(value)) {
    return Object.entries(value as Record<string, unknown>).reduce(
      (acc, [key, child]) => ({
        ...acc,
        ...flattenWorkerTokens(child, prefix ? `${prefix}-${key}` : key),
      }),
...
function handleTool(name: string, args: Record<string, unknown>) {
  ...
  if (name === "axint.swift.validate") {
    ...
    if (diagnostics.length === 0)
      return textResult("No issues found. Swift source is clean.");
    const output = diagnostics
      .map(
        (d) =>
          `[${d.code}] ${d.severity}: ${d.message}${d.line ? ` (line ${d.line})` : ""}${d.suggestion ? `\n   fix: ${d.suggestion}` : ""}`
      )
      .join("\n");
  ...
  if (name === "axint.swift.fix") {
    ...
    if (result.fixed.length > 0) {
      ...
      parts.push(
        `Applied ${result.fixed.length} fix(es)${passes}:\n${result.fixed.map((f) => `  - [${f.code}] ${f.message}`).join("\n")}`
      );
    }
